# Collaborative Open Development Environment (CODE)

Welcome to the **CODE** repository, a central hub for transportation-related artificial intelligence models, data pipelines, and research implementations.

Please see the <a href="https://tfhrc-ai-code.github.io/AI-CODE/" target="_blank" rel="noopener noreferrer">user guide</a>.

---

## Overview

This repository hosts AI case studies, pre-trained model weights, and field-tested codebases across key transportation domains:

* **Infrastructure Inspection & NDE** (e.g., Bridge Deck Crack Detection)
* **Safety & Operations** (e.g., Work Zone Monitoring)
* **Traffic Operations & Planning**

---

## How to Contribute

We welcome contributions from state DOTs, researchers, and industry partners:

1. Review the **<a href="https://tfhrc-ai-code.github.io/AI-CODE/contribution-guide/" target="_blank" rel="noopener noreferrer">Contribution Guide</a>** on our documentation site.
2. Format your case study using the template located at `docs/templates/case-study-template.md`.
3. Open a Pull Request against the `main` branch.
